# 🗂️ Frentes de Trabalho — Tecnologia Takwara

> Documento mestre de administração das frentes. Cada frente é uma linha de trabalho independente. **Não misturar assuntos, não cruzar repositórios.**
> Mantido pelo Hermes Agent · Atualizado em 01/07/2026 (tarde)

---

## ⚠️ Como Usar Este Documento

1. **Toda conversa começa com** — Hermes consulta este documento e te atualiza: "Status das frentes hoje:..."
2. **Para baixar material** — use o caminho de TRIAGEM da frente correspondente
3. **Para consultar** — cada frente tem seu repositório, seu escopo, suas regras de fronteira
4. **Para criar nova frente** — Fabio autoriza, Hermes executa

---

## 📊 Raio-X Geral (01/07/2026)

### Resumo da Sessão de Hoje (01/07) — Fábrica Modelo

| Ação | Resultado |
|------|-----------|
| **Resultado Gemini — Perfis ECOSALA** | 3 novas fichas (Paron, Gisele, Daniela) + Vicente confirmado. README e Acervo index atualizados com Membros Ativos (8) e Prospecção (18) |
| **Resultado Gemini — ICTs Multiplicadoras (AC, BA, RJ, SC, GO)** | 5 novos perfis fichados (Marcondes/UFAC, Guilherme/UFBA, Romildo/UFRJ, Humberto/UFSC, Alan/IF Goiano) + 5 cartas-convite Tipo B criadas |
| **Cartas Tânia + Ludmila** | Tipo A personalizadas criadas |
| **Carta Vicente** | Tipo A criada para formalizar participação no ECOSALA |
| **Emissários definidos** | André (Bliska, Rocco, Carvalho, Romildo) · Tânia (Marcondes, Guilherme) · Vicente (Humberto, Alan) · Fabio (Tânia, Ludmila, Vicente) |
| **👥 ECOSALA — Quadro de Pessoas** | Seção reformulada: Membros Ativos com colunas Enviado por/Resposta. Prospecção em Andamento com 18 pessoas e 5 status (✅ 📄 🔍 🔄 ⏳) |
| **Protocolo de envio** | Regra fixa: carta redigida ≠ carta enviada. Consentimento grupo gestor → envio com Cc → atualização. Prazo máx: **03/07** |
| **Dinâmica do ecossistema** | README agora aberto para visitantes indicarem contatos, redigirem cartas ou serem emissários |
| **Cartas padronizadas** | Todas as 11 cartas com header De/Para/Email/Cc/Data + Cc obrigatório para Fabio |
| **Prompt revisado** | `prompt-hermes-icts-multiplicadoras.md` com escaping limpo, status corrigidos, checkpoint de validação |

### Lembretes recorrentes ativos no cron:
- **Diário 08:00** — Fábrica Modelo: cartas pendentes de envio (prazo 03/07)
- **Diário 08:00** — Pendências a resolver (Lattes perdidos, CPF/LinkedIn do Paron)

---

## ⚠️ Como Usar Este Documento

1. **Toda conversa começa com** — Hermes consulta este documento e te atualiza: "Status das frentes hoje:..."
2. **Para baixar material** — use o caminho de TRIAGEM da frente correspondente
3. **Para consultar** — cada frente tem seu repositório, seu escopo, suas regras de fronteira
4. **Para criar nova frente** — Fabio autoriza, Hermes executa

---

## 📊 Raio-X Geral (30/06/2026)

### Resumo da Sessão de Hoje (30/06)

| Ação | Resultado |
|------|-----------|
| **Reunião Fábrica Modelo (André/Maurilho/Michel)** | Organizada, transcrição decupada, ATA produzida, 7 blocos do roteiro de defesa contra concreto, cartas de intenção (Michel, Imperveg), mensagens de divulgação, formulário-espelho FINEP, materiais extraídos do Drive do André (6/10 PDFs) |
| **Cartas de intenção reescritas** | Michel e Imperveg agora como documentos do grupo executor, não cartas de Fabio — vínculo de assessoria, não fornecimento |
| **Repositório ludmila-athis-df** | Criado, AGENTS.md, README.md, .gitignore. Ficha da Ludmila no Acervo com Lattes encontrado (K4127950D7). Decupagem da conversa 2021-2025 (7 atos, 585 linhas). 30 áudios transcritos (810 linhas). Mensagem-rascunho para Ludmila com status + links |
| **Documentos legais UnB/Profa Tânia** | 6 estratégias legais (docs/dispositivos-legais-parceria-unb.md), minuta para procurador (docs/minuta-procurador-unb.md), refutação técnica à posição do DECANATO enviada |
| **Consolidação Acervo Científico** | Diretório fantasma "Analises e escrita cientifica" (sem acento, sem git) consolidado: 269 arquivos copiados para o repo git real. 70+ arquivos commitados. Links quebrados corrigidos (4 ocorrências). REF_* movidos para docs/analises/00_outros/ |
| **Consolidação Vaga Lúmen** | PDFs de proposta movidos para pastas corretas. Estrutura padronizada. |
| **Organização geral** | .gitignore padronizado em todos os repos. Raiz limpa (só AGENTS.md, README.md, mkdocs.yml + pastas). 6 repos verificados (fabrica-modelo, ludmila-athis-df, unb-desafios, ECOSALA, eco-prancha, Personagens-Bambu) — nada misturado ✅ |
| **Eco Prancha** | .gitignore criado, 17 arquivos WhatsApp removidos do tracking (áudios, vídeos, fotos, chat.txt) |
| **Fichas no Acervo** | Perfil Fabio Takwara, Perfil André Blanco, Perfil Ludmila Correia, Protocolo Governança Cancún, Ficha Texos/Michel (rascunho). REF_* para certificações criadas na sessão anterior complementadas |

### Resumo da Sessão Anterior (29/06)

| Ação | Resultado |
|------|-----------|
| **Carta de Intenções LaPCiS** | Redigida, commitada, enviada à Profa Tânia — aguardando retorno |
| **Mensagem Ravi** | Aprovação obtida para vincular Ecolaborativa à UnB |
| **Áudio André (Fábrica Modelo)** | Transcrito, decupado. Edital FINEP baixado. Repositório criado. Pauta da reunião preparada |
| **Áudio Marcello (Eco Prancha)** | Transcrito, decupado. Estado da arte compilado. Repositório criado |
| **Certificações** | 7 fichas individuais criadas no acervo (LEED, AQUA, Casa Azul, VERRA, Gold Standard, ACV, Cimento) |
| **SMGA** | Ficha criada no acervo |
| **Tratamento Sem Veneno** | Ficha criada no acervo |
| **AGENTS.md** | Atualizado com novos atores e regras de fronteira |
| **README Mentoria** | Atualizado com repos e status |
| **README Acervo** | Atualizado com últimas adições |

### Mapa de Todos os Diretórios

| # | Diretório | Git | GH Pages | .md | Status |
|---|-----------|-----|----------|-----|--------|
| 1 | **Mentoria_Tecnologia_Takwara** | ✅ | ✅ | ~210 | ★ Ativo (Master) |
| 2 | **ECOSALA** | ✅ | ✅ | ~25 | ★ Ativo |
| 3 | **fundo-vaga-lumen-2026** | ✅ | ❌ | ~40 | ⏳ Aguardando reunião André |
| 4 | **plataforma-juventude-solidaria-2026** | ✅ | ✅ | ~53 | ★ Ativo |
| 5 | **Analises-e-escrita-cientifica** | ✅ | ✅ | ~395 | ★ Ativo (+65 novas fichas/docs) |
| 6 | **Personagens-Bambu** | ✅ | ❌ | ~22 | ★ Publicado |
| 7 | **Takwara-Tech** ⚠️ | ✅ (Resck) | ✅ | ~182 | 🔴 Triagem concluída |
| 8 | **Mulheres-Tecem-Amazonia** | ✅ | ✅ | ~1.267 | ⚡ Latente |
| 9 | **fabrica-modelo** 🆕 | ✅ | ❌ | ~17 | 🟢 Reunião realizada (30/06) |
| 10 | **eco-prancha** 🆕 | ✅ | ❌ | ~2 | 🟢 Aguardando call Marcello |
| 11 | **unb-desafios-amazonia-2026** | ✅ | ✅ | ~18 | ⏳ Pré-proposta 01/09 |
| 12 | **ludmila-athis-df** 🆕 | ✅ | ❌ | ~35 | 🟢 Mensagem pronta, aguardando envio |

---

## 🏁 As Frentes de Trabalho

### FRENTE 1 — MENTORIA (MAESTRO)

| Campo | Valor |
|-------|-------|
| **Repositório** | Mentoria_Tecnologia_Takwara |
| **GH Pages** | ✅ |
| **Status** | ★ Ativo |

🎯 Regência do ecossistema. AGENTS.md, FRENTES_DE_TRABALHO.md, 7 módulos TEC, MANUAL_OPERACAO.md.

⛔ Não incluir fichas científicas (vão pro Acervo).

---

### FRENTE 2 — ECOSALA

| Campo | Valor |
|-------|-------|
| **Repositório** | ECOSALA |
| **GH Pages** | ✅ |
| **Status** | ★ Ativo |

👥 Marcos Paron (IFSP), Gisele Vilela (Embrapa), André Blanco (TEIA), Joaquim Sando (MST), Vicente Borges (IFB), Daniela Maciel (Embrapa), Murilo Miguel (Terra Viva).

---

### FRENTE 3 — VAGA LÚMEN (FINEP)

| Campo | Valor |
|-------|-------|
| **Repositório** | fundo-vaga-lumen-2026 |
| **GH Pages** | ❌ |
| **Status** | ⏳ Aguardando reunião André |

🎯 Proposta FINEP Mais Inovação — Linha 4 (Moradia Sustentável).

⛔ **SP, proponente separado da Fábrica Modelo. NÃO misturar.**

---

### FRENTE 4 — MST JUVENTUDE SOLIDÁRIA

| Campo | Valor |
|-------|-------|
| **Repositório** | plataforma-juventude-solidaria-2026 |
| **GH Pages** | ✅ |
| **Status** | ★ Ativo |

👥 Joaquim Sando (MST), Murilo Miguel (Terra Viva).

---

### FRENTE 5 — ACERVO CIENTÍFICO

| Campo | Valor |
|-------|-------|
| **Repositório** | Analises-e-escrita-cientifica |
| **GH Pages** | ✅ |
| **Status** | ★ Ativo (+65 novas entradas em 30/06) |

🎯 **Fonte ÚNICA de referências para TODAS as frentes.** Fichas seguem protocolo próprio de curadoria documental (8 seções), em equivalência com protocolos acadêmicos.

⛔ Referências sempre do próprio acervo — nunca Zenodo, sites externos ou fontes não verificadas.

**Últimas adições (30/06):**
- Perfil Fabio Takwara + ORCID
- Perfil André Blanco + Lattes (extraído Drive)
- Perfil Ludmila Correia + Lattes K4127950D7
- Protocolo Governança Cancún (Salvaguardas REDD+)
- Ficha Texos/Michel (rascunho — aguardando confirmação de grafia)
- 269 arquivos consolidados do diretório fantasma
- REF_* movidos para docs/analises/00_outros/

---

### FRENTE 6 — PERSONAGENS-BAMBU

| Campo | Valor |
|-------|-------|
| **Repositório** | Personagens-Bambu |
| **GH Pages** | ❌ |
| **Status** | ★ Publicado |

---

### FRENTE 7 — TAKWARA-TECH (LEGACY)

| Campo | Valor |
|-------|-------|
| **Repositório** | Resck/Takwara-Tech |
| **Status** | 🔴 Triagem concluída |

Triagem completa: 83 PDFs extraídos, 15 áudios processados, conteúdo distribuído ao Acervo.

---

### FRENTE 8 — MULHERES BIOECONOMIA AMAZÔNIA / MQTF

| Campo | Valor |
|-------|-------|
| **Repositório** | Mulheres-Tecem-Amazonia |
| **Status** | ⚡ Latente |

📌 MQTF é sigla INTERNA — em docs externos referenciar como "projeto de bioeconomia amazônica do consórcio UnB/UFAC/UFRR".

---

### FRENTE 9 — UnB / DESAFIOS AMAZÔNIA

| Campo | Valor |
|-------|-------|
| **Repositório** | unb-desafios-amazonia-2026 |
| **Status** | ⏳ Pré-proposta 01/09/2026 |

👥 Profa Tânia (LaPCiS — FUP UnB Planaltina).

📄 Documentos produzidos hoje:
- docs/dialogo-tania-fabio-3006.md (alinhamento)
- docs/analise-marcos-legais-voluntariado-unb.md
- docs/dispositivos-legais-parceria-unb.md (6 estratégias)
- docs/minuta-procurador-unb.md
- docs/transcricoes/ (transcrição + resposta + mensagem refutação)

⛔ Carreira pessoal de Fabio — não misturar com outros projetos.

---

### FRENTE 10 — FÁBRICA MODELO

| Campo | Valor |
|-------|-------|
| **Repositório** | fabrica-modelo |
| **Criado** | 29/06/2026 |
| **Status** | 🟢 11 cartas prontas · Aguardando envio (prazo 03/07) |

👥 André Blanco (TEIA), Maurilho (habitação social, vice-presidente FNA), Michel (indústria — Texos).

📄 Documentos do projeto (em fabrica-modelo/docs/):
- ATA_REUNIAO_3006.md
- ORGANIZACAO_REUNIAO_3006.md (11 tarefas T01-T11, prazos 5-7d)
- ROTEIRO_DEFESA_CONTRA_CONCRETO.md (6 blocos)
- carta-intencao-michel.md / carta-intencao-imperveg.md
- carta-modelo-apresentacao-parceiros.md
- formulario-espelho-finep.md
- mensagem-divulgacao-grupo.md
- mensagem-whatsapp-simplificada.md
- proposal-teaser.md
- materiais-andre/ (6 .md extraídos do Drive)
- transcricoes/TRANSCRICAO_REUNIAO_3006.md

📄 **Produzido em 01/07:**
- **Acervo:** 8 novas fichas (Paron, Gisele, Daniela, Marcondes, Guilherme, Romildo, Humberto, Alan)
- **Cartas:** 11 cartas-convite redigidas (Tânia, Ludmila, Vicente, Bliska, Rocco, Carvalho, Marcondes, Romildo, Guilherme, Humberto, Alan)
- **README:** Quadro ECOSALA ativos/prospecção, dinâmica do ecossistema, protocolo de envio, 5 status
- **ICTs SP/MG:** Tabela atualizada com 8 ICTs (⭐3-5)
- **ICTs Multiplicadoras:** Nova tabela AC/BA/RJ/SC/GO
- **Prompt revisado:** `prompt-hermes-icts-multiplicadoras.md`

📌 **Protocolo de envio de cartas:**
1. Consentimento do grupo gestor
2. Envio pelo emissário designado com Cc para Fabio
3. Atualização da tabela (📄 → ✅)
4. Prazo máximo: **03/07/2026**

📌 **Ponto crítico:** Contrapartida mínima FINEP = R$5.000.000,00. Michel dispõe de ~R$160.000 (insuficiente). Buscar alternativas (cooperativa de crédito, consórcio com TEIA e parceiros).

⛔ **SP, sem UnB, sem LaPCiS.** Proponente separado da Vaga Lúmen.

---

### FRENTE 11 — ECO PRANCHA

| Campo | Valor |
|-------|-------|
| **Repositório** | eco-prancha |
| **Criado** | 29/06/2026 |
| **Status** | 🟢 Aguardando call Marcello |

👥 Marcello Pedro (Grupo Raízes, RJ).

📄 Estado da arte completo em docs/01_PESQUISA/. .gitignore corrigido (17 arquivos WhatsApp removidos do tracking).

⛔ **RJ, sem relação com SP ou UnB.**

---

### FRENTE 12 — CARREIRA / LAPCIS (pessoal)

| Campo | Valor |
|-------|-------|
| **Repositório** | (vinculado à carreira de Fabio) |
| **Status** | ⏳ Aguardando resposta Profa Tânia + retorno procurador |

📄 Documentos encaminhados:
- Carta de intenções (via unb-desafios)
- Refutação técnica ao DECANATO de Extensão
- 6 estratégias legais para contratação (Ecolaborativa)
- Minuta para procurador da UnB

⛔ **Frente pessoal. NÃO misturar com projetos parceiros.**

---

### FRENTE 13 — LUDMILA / ATHIS-DF 🆕

| Campo | Valor |
|-------|-------|
| **Repositório** | ludmila-athis-df |
| **Criado** | 30/06/2026 |
| **Status** | 🟢 Mensagem pronta, aguardando Fabio aprovar/envio |

👥 Ludmila de Araújo Correia (Arquiteta CAU, LaPeCFaS UnB — parceria Xukuru, Be Viver).

📄 Documentos:
- docs/contexto-historico-xukuru.md
- docs/transcricoes/decupagem-conversa-2021-2025.md (7 atos, 585 linhas)
- docs/transcricoes/decupagem-seminario-athis-2024-2026.md
- docs/transcricoes/audios/ (30 transcrições, 810 linhas)
- docs/mensagem-ludmila.md

🎯 Não frente ativa — manter contexto histórico documentado para eventual retomada.

⛔ Foco exclusivo na parceria ATHIS/CAU DF, não frente de mobilização.

---

## 📋 Prioridades (01/07/2026)

| Prioridade | O quê | Status |
|------------|-------|--------|
| 🔴 **1** | Fábrica Modelo — enviar 11 cartas-convite (prazo 03/07) | ⏳ 11 cartas prontas · 0 enviadas |
| 🔴 **2** | Fábrica Modelo — definir contrapartida (R$5M mínimo) | ⏳ Michel ~R$160K insuficiente |
| 🔴 **3** | UnB/Desafios Amazônia — aguardar Procuradoria + Profa Tânia | ⏳ Aguardando |
| 🟡 **4** | Eco Prancha — call com Marcello | ⏳ Aguardando |
| 🟡 **5** | Imperveg (Donizete) — scouting de materiais | ⏳ Em triagem |
| 🟡 **6** | Vaga Lúmen — aguardar André definir prioridade vs Fábrica Modelo | ⏳ Aguardando André |
| 🔵 **7** | Acervo Científico — completar fichas pendentes (Michel, Lattes perdidos) | ⏳ Em standby |
| 🔵 **8** | Mentoria — preparar módulos TEC restantes | ⏳ Em segundo plano |

## 📋 Pendências / Itens de Regularização

| Item | Repositório | O quê fazer | Urgência |
|------|-------------|-------------|----------|
| P01 | Acervo Científico | Confirmar grafia "Texos" (Michel) com André para finalizar ficha | 🟡 Média |
| P02 | fabrica-modelo | Baixar 4 PDFs do Drive André (VITAU 164MB, Estudos Viabilidade) | 🟡 Média |
| P03 | Mentoria | Remover do disco diretório fantasma "Analises e escrita cientifica" | 🔴 Alta |
| P04 | eco-prancha | Verificar se há outros arquivos não-md no tracking (pós .gitignore) | 🟢 OK |
| P05 | unb-desafios | Preparar SPOA / proposta completa para 01/09 | 🟡 Média |
| P06 | ludmila-athis-df | Fabio aprovar mensagem antes do envio | 🟡 Média |
| P07 | Geral | Transcrições de áudio NUNCA versionadas — verificar | 🟢 OK |
| **P08** 🆕 | **fabrica-modelo** | **Enviar 11 cartas-convite até 03/07** (coordenar com André, Tânia, Vicente) | 🔴 **Crítico** |
| **P09** 🆕 | **Acervo Científico** | **3 Lattes não localizados** (Guilherme UFBA, Humberto UFSC, Alan IF Goiano) — solicitar aos pesquisadores | 🟡 Média |
| **P10** 🆕 | **Acervo Científico** | **CPF/LinkedIn C1-C4 (ECOSALA)** — solicitar ao Marcos Paron | 🟡 Média |
| **P11** 🆕 | **fabrica-modelo** | **E-mail Ludmila Correia** — ainda não confirmado (marcado como pendente) | 🟡 Média |
| **P12** 🆕 | **fabrica-modelo** | **Criar cartas cooperação técnica para Imperveg, Kehl, Purcom** (modelo de acordo pronto em docs/editais/) | 🟡 Média |
| **P13** 🆕 | **Acervo Científico** | **67 arquivos do MQTF copiados** (30 SCI + 31 FICHA + 6 LAB) — pendente de padronização para o protocolo de curadoria (8 seções) | 🟡 Média |

---

## 🤖 Regras de Memória e Fronteira

- Referências SEMPRE do acervo científico, nunca Zenodo/externo
- LaPCiS = só carreira pessoal, nunca citar em projetos parceiros
- Cada parceiro isolado: André, Marcello, Tânia, Ludmila — não cruzar
- Fichas = referência geral, sem vínculo a projeto específico
- Siglas (SMGA, MPTDF, T01) sempre com nome completo + link ficha
- Dúvida? Perguntar antes de assumir/inventar
- 🚫 **NUNCA citar o repositório Takwara-Tech** em fichas, cartas ou documentos externos. Fontes dele são "documento interno de referência" ou "acervo próprio do autor".

### 🚫 Regras de Fronteira Entre Repositórios

| Repositório | Guarda | NÃO guarda |
|-------------|--------|------------|
| **Mentoria_Tecnologia_Takwara** (master) | AGENTS.md master, FRENTES_DE_TRABALHO.md, MANUAL_OPERACAO.md, 7 módulos TEC, Jornada 7 passos, Metodologia, PLANOS de frentes (exceto planos de reunião de projetos específicos), documentação do ecossistema | Fichas científicas (vão pro Acervo), documentos de projetos parceiros (vão pros seus repos), transcrições de reunião de projetos específicos |
| **Analises-e-escrita-cientifica** (acervo) | Fichas científicas (8 seções), análises, referências (REF_*), fichas técnicas de tecnologias (PU Vegetal, etc.), perfil de pesquisadores | Documentos de gestão de projetos, atas de reunião, materiais de mentoria |
| **fabrica-modelo** (projeto) | Edital FINEP, atas/transcrições/organização das reuniões, pautas do projeto, cartas de intenção, documentos específicos da Fábrica Modelo | Fichas científicas, documentação geral do ecossistema, planos de outras frentes |
| **fundo-vaga-lumen-2026** (projeto) | Documentos específicos da proposta Vaga Lúmen | Tudo que não é da proposta |
| **eco-prancha** (projeto) | Documentos específicos do projeto Eco Prancha | Tudo que não é do projeto |
| **ludmila-athis-df** (projeto) | Contexto histórico Ludmila, decupagens, transcrições, mensagens | Documentos de outros projetos, fichas científicas |
| **Personagens-Bambu** | Personagens, biotipos | Fichas técnicas, documentos de projeto |
| **unb-desafios-amazonia-2026** | Proposta Desafios da Amazônia, documentos legais UnB | Carreira LaPCiS (é pessoal, mas coabitam no mesmo repo), outros projetos |
| **ECOSALA** | Atas, editais, fichas do grupo | Documentos de projetos parceiros que não são do coletivo |
| **plataforma-juventude-solidaria-2026** | Documentos do projeto MST Mário Lago | Fichas científicas, outros projetos |

### ⚡ Regras de Ouro

1. **Um documento, um repositório** — cada arquivo pertence a UM lugar. Se está em dois, um está errado.
2. **Fichas científicas SEMPRE no Acervo** — inclusive fichas técnicas de tecnologias (PU Vegetal, forno MPTDF, etc.)
3. **Planos de reunião de projetos específicos** → repositório do projeto, não na Mentoria
4. **Transcrições de reunião** → repositório do projeto (docs/transcricoes/)
5. **Material bruto de triagem (TRIAGEM_BRUTA)** → Mentoria (é curadoria do ecossistema)
6. **Referências científicas (REF_*)** → Acervo Científico
7. **PROPOSIÇÃO DE FABIO NÃO É FORNECEDOR** — nunca colocar documentos que sugiram vínculo comercial
8. **Transcrições de áudio NUNCA versionadas** — só pautas e ATAS vão pro git

---

> *Documento mantido pelo Hermes Agent · Tecnologia Takwara · 30/06/2026*
> *Consulte sempre no início de cada sessão para saber onde estamos.*
